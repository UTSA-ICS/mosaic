...
from mosaic.openstack.common import strutils
...
MOSAIC_TEST_SOCKET_FD_STR = 'MOSAIC_TEST_SOCKET_FD'
...
def add_features_to_http_headers(features, headers):
    """
    Adds additional headers representing mosaic features to be enabled.
...
class PrettyTable(object):
    """Creates an ASCII art table for use in bin/mosaic
    ...
    def __get_backend(self):
        if not self.__backend:
            ...
            if backend_name not in self.__backends:
                ...
                raise exception.MosaicException(msg)
...
def get_test_suite_socket():
    global MOSAIC_TEST_SOCKET_FD_STR
    if MOSAIC_TEST_SOCKET_FD_STR in os.environ:
        fd = int(os.environ[MOSAIC_TEST_SOCKET_FD_STR])
        sock = socket.fromfd(fd, socket.AF_INET, socket.SOCK_STREAM)
        sock = socket.SocketType(_sock=sock)
        sock.listen(CONF.backlog)
        del os.environ[MOSAIC_TEST_SOCKET_FD_STR]
        os.close(fd)
        return sock
    return None
